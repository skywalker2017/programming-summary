...
+ create2 address calculation: address = keccak256(0xff ++ sender_address ++ salt ++ keccak256(init_code))[12:]
+ why there is a 0xff prefix when using create2: to previde collide with contract generated by create; prevents protential attack where different hash input produce the same output
...
```
+ This variable is initialized at deployment time with the address of the contract itself.
+ The value is computed during contract creation and then embedded directly in the contract bytecode.
...
```
+ Won't corrupt memory: Your code won't write to memory locations that could corrupt Solidity's memory management
+ Respects memory layout: You won't interfere with Solidity's memory layout or the free memory pointer
+ Follows allocation rules: If you allocate memory, you properly update the free memory pointer at 0x40
+ Preserves memory invariants: You maintain all the memory invariants that Solidity expects+ 
...
+ transfer send forward 2300 gas when using transfer and it got revert if failed, there are two conditions that might got revert
  + gas not enough
...
+ the main reason is that in transparent pattern, every function have to check whether its an admin call. For a frequently called contract UUPS is much cheaper

#### why can't dynamically change the length of an array stored in memory
...
```
// Make the call without pre-allocating return memory
let success := delegatecall(gas(), implementation, 0, calldatasize(), 0, 0)
// Get the exact size of return data
let size := returndatasize()
// Copy the return data to memory
returndatacopy(0, 0, size)
// Return or revert with the exact data
switch success
